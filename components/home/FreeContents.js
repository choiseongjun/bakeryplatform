...
const FreeContents = ({navigation,modalOpen,setModalOpen}) => {

    const [freeContentData,setFreeContentData] = useState([]);

    useFocusEffect(
        React.useCallback( () => {
            axios.get('/contentList/2')
            .then(function (response) {
    
                setFreeContentData(response.data);
            })
